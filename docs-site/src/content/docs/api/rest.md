---
title: REST API
description: Driving QAP from your own code — Service Token auth, and where the interactive reference lives
---
...
Your own QAP serves it:

```
https://<your-qap-host>/api/docs
```
...
Paths are relative to your host, so `GET $QAP_BASE_URL/api/workspaces` lists workspaces.
...
Rather than reading operation lists yourself, hand them to an agent: the [qap-api skill](/api/skills/) is generated from this same spec, and it's what lets a local agent drive QAP.
